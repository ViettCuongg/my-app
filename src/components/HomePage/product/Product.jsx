      ...
      <div className={css.products}>
        <ul className="bg-white rounded-lg shadow-lg  list-none flex flex-col font-medium text-[20px] h-[25rem] overflow-y-scroll w-1/5">
          ...
          {faculty.map((item,index)=>(
            <li key={index} className={`px-3 py-4 text-base cursor-pointer hover:bg-[#E0FFFF] ${activeIndex === index ? 'bg-[#E0FFFF]' : ''}`} onClick={() => filter(item.id,index)}>{item.name}</li>
          ))}
        </ul>
            ...
            {majors.map((item,index)=>(
              <div className="flex shadow-xl bg-white h-[200px] gap-4" key={index}>
                ...
                <div className="flex flex-col flex-1 py-2 pe-3">
                  ...
                  <div className="h-[62px] w-full leading-8 my-4 line-clamp-3 font-normal text-base"> <span className="font-bold">Mã ngành:</span> {item.code}</div>
                  
                  <Link to={`/detail/${item.id}`} className="py-1 mt-2 text-center text-white border rounded-lg cursor-pointer flex-end bg-slate hover:bg-opacity-85">View Detail</Link>
                </div>
              </div>
            ))}
