    ...
    <div className={css.container}>
      ...
      <div className={css.products}>
        <ul className="bg-white rounded-lg shadow-lg  list-none flex flex-col font-medium text-[20px] h-[25rem] overflow-y-scroll w-1/5">
          <li
            className={`px-3 py-4 hover:bg-[#E0FFFF] cursor-pointer ${activeIndex === null ? 'bg-[#E0FFFF]' : ''
              }`}
            onClick={() => {
          ...
          {faculty.map((item, index) => (
            <li
              key={index}
              className={`px-3 py-4 text-base cursor-pointer hover:bg-[#E0FFFF] ${activeIndex === index ? 'bg-[#E0FFFF]' : ''
                }`}
